...
server_up_healthy_and_clean()
{
  ...
  export CONTAINER_NAME="${CYBER_DOJO_MODEL_SERVER_CONTAINER_NAME}"
  export CONTAINER_PORT="${CYBER_DOJO_MODEL_PORT}"
...
{
  if [ "${1:-}" != 'server' ]; then
    export SERVICE_NAME=model-client
    export CONTAINER_NAME="${CYBER_DOJO_MODEL_CLIENT_CONTAINER_NAME}"
    export CONTAINER_PORT="${CYBER_DOJO_MODEL_CLIENT_PORT}"
    augmented_docker_compose up \
      --detach \
      "${SERVICE_NAME}"
    exit_non_zero_unless_healthy
